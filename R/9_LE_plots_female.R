...
LE_Gompertz_PH_45 <- dplyr::filter(LE_Gompertz_PH_45, !is.na(mean_female)) %>%
   select(country, year, ll_female, mean_female, ul_female) %>%
   rename(ll_pol = ll_female, expect_pol = mean_female, ul_pol = ul_female) %>%
...
stats.pol <- dplyr::filter(stats, Population == 'Politicians') %>%
   mutate(year = 1920, expect = 55, # Coordinates of the max-min box for politicians
...
stats.gen <- dplyr::filter(stats, Population == 'General population') %>%
   mutate(year = 2003, expect = 29, # Coordinates of the max-min box for general population
          label = paste(max, '\n', 
                        min,
                        sep = '') 
   )
...
legend$expect[1:2] <- 49                                     # y coordinates for politician legend line
legend$years[3:4] <- c(start.gen, start.gen + line.length)   # x coordinates for general population legend line
legend$expect[3:4] <- 23                                     # y coordinates for general population legend line
...
p <- ggplot(data = plot.data.legend, aes(x = years, y = expect, ymax = UL, ymin = LL, color = Population), size = 0.01) +
   ...
   scale_y_continuous(name = 'Life expectancy at age 45', limits = c(20, 55)) +
   ...
   geom_label(data = legend, aes(x = 1920, y = 55, label='Max (95% CI)\nMin (95% CI)'), size = legend.fs, color = 'blue', hjust = 0, vjust = 1) + # labels
   geom_text(data = legend, aes(x = start.pol - 1, y = 52, label = 'Politicians'), size = 1.1*legend.fs, color = 'blue', hjust = 0, vjust = 0) +
   
   geom_label(data = legend, aes(x = 2003, y = 29, label='Max\nMin'), size = legend.fs, hjust = 0, vjust = 1) + # labels
   geom_text(data = legend, aes(x = 1960, y = 26, label = 'General population'), size = 1.1*legend.fs, hjust = 0, vjust = 0)

p

jpeg('LE_at_45_female.jpg', width = 2800, height = 2800, res = 300, quality = 100)
print(p)
dev.off()  
...
LE_Gompertz_PH_45_boot <- LE_Gompertz_PH_45_boot %>%
   select(year, expect_female, country) %>%
   rename(expect_pol = expect_female) %>%
   mutate(country = as.character(country), country = replace(country, country == 'New_Zealand', 'New Zealand'))
...
p <- ggplot(data = plot.data.legend, aes(x = year, y = dif, ymin = ll_dif, ymax = ul_dif)) +
   ...
   geom_text(data=legend, aes(x = 1967, y = 5, label ='Politicians living longer'), size = legend.fs) +
   geom_text(data=legend, aes(x = 1967, y = 0.15, label='Politicians equal to\n general population'), col = 'dark red', size = legend.fs) +
   geom_text(data=legend, aes(x = 1967, y = -5, label='Politicians living shorter'), size = legend.fs)

p

jpeg('LE_GAP_at_45_female.jpg', width = 2800, height = 2800, res = 300, quality = 100)
print(p)
dev.off()
